...
RoboligoConnectorMavros::on_set(RobotState & robot_state)
{
  ...
  RCLCPP_INFO(node->get_logger(), "Mavros Connector offboard set --> %s",
      offboarding_value.c_str());
  RCLCPP_INFO(node->get_logger(), "Mavros Connector StandBy set --> %s", standingby_value.c_str());

  std::function<void()> arming_callback = std::bind(&RoboligoConnectorMavros::arming_callback,
      this);
  robot_state.get_trigger("arming")->register_callback(arming_callback);

  std::function<void()> disarming_callback = std::bind(&RoboligoConnectorMavros::disarming_callback,
      this);
  robot_state.get_trigger("disarming")->register_callback(disarming_callback);

  std::function<void()> takingoff_callback = std::bind(&RoboligoConnectorMavros::takingoff_callback,
      this);
  robot_state.get_trigger("takeoff")->register_callback(takingoff_callback);

  std::function<void()> landing_callback = std::bind(&RoboligoConnectorMavros::landing_callback,
      this);
  robot_state.get_trigger("landing")->register_callback(landing_callback);

  std::function<void()> offboarding_callback =
    std::bind(&RoboligoConnectorMavros::offboarding_callback, this);
...
RoboligoConnectorMavros::arming_callback()
{
  ...
  while (!arming->wait_for_service(2s)) {
    ...
    RCLCPP_INFO_STREAM(node->get_logger(),
        "service " << arming_interface->get_topic() << " not available, waiting again...");
  }
...
RoboligoConnectorMavros::disarming_callback()
{
  ...
  while (!disarming->wait_for_service(2s)) {
    ...
    RCLCPP_INFO_STREAM(node->get_logger(),
        "service " << disarming_interface->get_topic() << " not available, waiting again...");
  }
...
RoboligoConnectorMavros::takingoff_callback()
{
  ...
  while (!takingoff->wait_for_service(2s)) {
    ...
    RCLCPP_INFO_STREAM(node->get_logger(),
        "service " << takingoff_interface->get_topic() << " not available, waiting again...");
  }
...
RoboligoConnectorMavros::landing_callback()
{
  ...
  while (!landing->wait_for_service(2s)) {
    ...
    RCLCPP_INFO_STREAM(node->get_logger(),
        "service " << landing_interface->get_topic() << " not available, waiting again...");
  }
...
RoboligoConnectorMavros::offboarding_callback()
{
  ...
  while (!offboarding->wait_for_service(2s)) {
    ...
    RCLCPP_INFO_STREAM(node->get_logger(),
        "service " << offboarding_interface->get_topic() << " not available, waiting again...");
  }
...
RoboligoConnectorMavros::standingby_callback()
{
  ...
  while (!standingby->wait_for_service(2s)) {
    ...
    RCLCPP_INFO_STREAM(node->get_logger(),
        "service " << standingby_interface->get_topic() << " not available, waiting again...");
  }
...
RoboligoConnectorMavros::set_data_loss_exception(int value)
{
  ...
  while (!params_change->wait_for_service(2s)) {
    ...
    RCLCPP_INFO_STREAM(node->get_logger(),
        "service " << params_interface->get_topic() << " not available, waiting again...");
  }
